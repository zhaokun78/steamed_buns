  ...
  <view class="uni-container">
    <uni-forms ref="form" :value="formData" validate-trigger="submit" err-show-type="toast">
      ...
      <uni-forms-item name="address" label="详细地址" required>
        <uni-easyinput placeholder="详细地址" v-model="formData.address"></uni-easyinput>
      </uni-forms-item>
      <uni-forms-item name="longitude" label="经度" required>
        <uni-easyinput placeholder="经度" v-model="formData.longitude"></uni-easyinput>
      </uni-forms-item>
      <uni-forms-item name="latitude" label="纬度" required>
        <uni-easyinput placeholder="纬度" v-model="formData.latitude"></uni-easyinput>
      </uni-forms-item>
      ...
      <uni-forms-item name="business_hour_start" label="每天几点开始营业" required>
        <slider :min="0" :max="23" :show-value="true" @change="binddata('business_hour_start', $event.detail.value)" :value="formData.business_hour_start"></slider>
      </uni-forms-item>
      <uni-forms-item name="business_hour_end" label="每天几点结束营业" required>
        <slider :min="0" :max="23" :show-value="true" @change="binddata('business_hour_end', $event.detail.value)" :value="formData.business_hour_end"></slider>
      </uni-forms-item>
...
<script>
  import { validator } from '../../js_sdk/validator/wfy-shop.js';
  ...
  export default {
    ...
    methods: {
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("type,name,city_code,address,longitude,latitude,phone,business_hour_start,business_hour_end,business_state,delivery_amount").get().then((res) => {
