<template>
	<view class="container">
		<view class="main">
			...
			<template v-else>
				<uni-swipe-action>
					<uni-swipe-action-item class="address-wrapper" :right-options="swipeOption" @click="handleSwipeClick(address._id)"
						...
						<view class="address" @tap="chooseAddress(address)">
							<view class="left flex-fill overflow-hidden mr-20">
								...
								<view class="font-size-sm text-color-assist">
									{{ address.name }} {{ address.sex==0 ? '先生' : '女士' }} {{ address.mobile }}
								</view>
							</view>
...
<script>
	...
	export default {
		...
		async onLoad({
			...
			let ret = await db.collection('uni-id-address').where('user_id == $cloudEnv_uid').get()
			console.log('uni-id-address', ret)
			this.addresses = ret.result.data
				...
				uni.showModal({
					...
					success: res => {
						if (res.confirm) {
							...
							const db = uniCloud.database()
							db.collection('uni-id-address').doc(id).remove().then((r) => {
								uni.hideLoading()

								const index = this.addresses.findIndex(item => item._id == id)
